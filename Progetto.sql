...
-- 1. Seleziona tutti i dati relativi ai goal.
SELECT * FROM Goal;

-- 2. Elimina la riga/tupla della tabella Squadra con l’attributo Antella.
DELETE FROM Squadra
WHERE Nome LIKE 'Antella';

-- 3. Seleziona il nome e il cognome di tutti i giocatori che giocano nella Lazio.
SELECT Nome,Cognome
FROM Giocatore JOIN ContrattoGiocatore ON Giocatore.NumTessera=ContrattoGiocatore.NumTessera
WHERE NomeSquadra LIKE 'Lazio';

/*
4. Seleziona il nome delle squadre che giocano in serie ‘A’ e che hanno fatto 
almeno 2 goal in una partita giocata fuori casa.
*/
...
/*
5. Seleziona il nome e il cognome dei giocatori che giocano in squadre di serie ‘A’ allenate da  
allenatori che hanno precedentemente allenato precisamente altre 11 squadre. 
...
-- 6. Preleva il numero totale dei goal fatti in casa dalla Fiorentina.
SELECT sum(GoalSquadraCasa) as GoalFattiInCasadallaFiorentina
FROM Partita
WHERE SquadraCasa = 'Fiorentina';  

/*
7. Selezionare per ogni partita giocata le informazioni su di essa
riguardante l'ultimo goal segnato.
*/
...
/*
8. Selezionare per ogni partita giocata le informazioni su di essa
riguardante il primo goal segnato. Queste in formazioni saranno in ordine
crescente in base ai minuti dei goal segnati.
*/
...
-- 9. Selezionare squadra casa, squadra ospite e i loro relativi goal, e per ogni goal le informazioni sul marcatore di esso
...
-- 10. Selezionare nome, cognome e squadra di tutti gli allenatori delle squadre
...
-- 11. Selezionare il numero totale di vittorie della Fiorentina contro le squadre di Roma. 
...
-- 12. Selezionare il numero dei tessera, nome e cognome dei portieri delle squadre che hanno subito più goal in una sola partita. 
...
-- 13. Trovare la media dei goal dalla squadra di casa, segnati in ogni giornata, escludendo le squadre di Torino; con il vincolo di aver segnato massimo 5 goal
...
-- 14. Selezionare numero tessera, cognome e nome del "capocannoniere" di ogni squadra, prendendo in considerazione solamente le prime dieci righe.
...
-- 15. Selezionare tutte le squadre che hanno vinto almeno una partita.
SELECT Squadra
FROM Vittorie
GROUP BY Squadra;

-- 16. Trovare i ruoli per i quali non esiste alcun giocatore che li ricopra nelle squadre di Roma.
...
-- 17. Selezionare nome, cognome e squadra in cui gioca, per ogni squadra, il giocatore più giovane.
...
-- 18. Classifica le partite in base ai goal effettuati.
...
-- 19. Calcola la media dei minuti di tutti i goal segnati. 
SELECT AVG(Minuto) AS MediaGoalMinuto
FROM Goal; 

-- 20. Calcola la classifica di tutte le partite giocate fino ad ora, presenti nell’archivio della partite.
